...
template<typename Rasterizer, typename VaryingGenerator>
void Stroker<Rasterizer, VaryingGenerator>::join(Line lastLeft, Line lastRight, Line left, Line right,
    ...
    if (joinStyle == BevelJoin) {
        ...
        const int steps = std::min(30, static_cast<int>(std::ceil(std::abs(arcLength / std::numbers::pi_v<float>))));
        assert(steps > 0);
